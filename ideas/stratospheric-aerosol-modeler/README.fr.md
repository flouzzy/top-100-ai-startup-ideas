...
```mermaid
graph TD
    A["Événement de Géo-ingénierie (Rogue)"] -->|"Injection d'aérosols stratosphériques"| B["Chaos Global Imprévisible"]
    B -->|"Moussons déviées / Famines"| C["Crise Mondiale"]
    A -->|"Stratospheric Aerosol Modeler"| D["Simulation Climatique Neural PDE"]
    D -->|"Prédictions Régionales Instantanées"| E["Réponse Stratégique de l'État"]
    style D fill:#a2c4c9,stroke:#333,stroke-width:2px
```
...
```mermaid
sequenceDiagram
    participant Gov as "Agence Gouvernementale"
    participant WorldModel as "Moteur World Model"
    participant HPC as "HPC Exascale / Earth System Models"
    participant AI as "Neural PDEs"
    Gov->>WorldModel: Saisie du vecteur d'injection (Lieu/Volume)
    WorldModel->>HPC: Initialisation des modèles physiques non-linéaires
    HPC->>AI: Envoi des données atmosphériques haute résolution
    AI-->>WorldModel: Prédiction instantanée des retombées régionales
    WorldModel-->>Gov: Évaluation des risques & stratégie
```
