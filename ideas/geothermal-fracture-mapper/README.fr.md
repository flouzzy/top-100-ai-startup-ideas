...
graph TD
    ...
    A["Le forage géothermique profond souffre d'un taux d'échec élevé (forages secs) en raison de la difficulté à cartographier avec précision les réseaux de fractures souterraines perméables sous 3km de profondeur, rendant le coût d'entrée prohibitif."] --> B["Modélisation de la sous-surface utilisant des modèles d'inversion sismique accélérés par Neural Physics (PINNs) pour créer un jumeau numérique prédictif de la fracturation des roches et de la circulation des fluides sous très haute pression et température."]
```
