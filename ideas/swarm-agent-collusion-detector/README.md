...
```mermaid
graph TD
    A["Autonomous AI Trading Agents"] -->|Hidden Cooperative Strategies| B["Algorithmic Collusion / Cartel"]
    B -->|Subtle Market Manipulation| C["Undetectable by Standard Compliance"]
    D["Inverse Game Theory Engine"] -->|Dynamic Graph Analysis| E["Behavioral Pattern Detection"]
    E -->|Alerts Regulators & Exchanges| F["Prevention of Systemic Market Failure"]
```
...
```mermaid
sequenceDiagram
    participant Agents as "AI Trading Swarm"
    participant Engine as "Collusion Detector"
    participant GT as "Inverse Game Theory Model"
    participant Compliance as "Market Regulator/Compliance"

    Agents->>Engine: Stream high-frequency transaction data
    Engine->>GT: Analyze decision spaces & RL policies
    GT->>GT: Detect non-competitive equilibrium patterns
    GT-->>Engine: Flag hidden cooperative strategies
    Engine-->>Compliance: Dashboard alert with mathematical proof of intent
```
