...
graph TD
    ...
    Probleme["État Actuel"] --> Solution["État Synthetic Data Quarantine"]
```
...
**La vérité cachée :** Un LLM ne peut pas s'auto-évaluer efficacement sur des pétaoctets de données pour détecter s'il a généré ou non une donnée. C'est un problème d'infrastructure de données massives (Big Data) et d'analyse probabiliste à grande échelle, nécessitant une tuyauterie dédiée et des algorithmes de détection spécifiques (détection de filigranes, analyse de distribution de tokens), et non un simple prompt.
