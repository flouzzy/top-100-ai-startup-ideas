...
```mermaid
graph TD
    A["Surface du Sol"] -->|Bunkers/Minerais Cachés| B["Vide/Densité Souterraine"]
    C["Essaim de Drones"] -->|Interférométrie à Atomes Froids| D["Capteurs Gravimétriques Quantiques"]
    D -->|Fusion Données Temps Réel| E["Moteur Cartographique IA"]
    E --> F["Carte 3D Haute Résolution du Sous-Sol"]
```
...
```mermaid
sequenceDiagram
    participant Swarm as "Essaim Drones"
    participant Sensor as "Gravimètre Quantique Miniature"
    participant Edge as "Fusion Edge IA"
    participant Base as "Centre de Commandement"

    Swarm->>Swarm: Vol en grille autonome
    Swarm->>Sensor: Stabilisation contre vibrations en vol
    Sensor->>Sensor: Mesures interférométrie atomes froids
    Sensor->>Edge: Données brutes d'anomalies gravitationnelles
    Edge->>Edge: Filtrage bruit & fusion multi-drones
    Edge-->>Base: Rendu carte de densité 3D en temps réel
```
