...
graph TD
    ...
    A["The insertion of hardware trojans or counterfeit chips into the semiconductor supply chain. Current optical or X-ray inspection methods are either destructive or too slow to examine each chip, leaving critical infrastructure vulnerable to physical kill switches undetectable at the software level."] --> B["Using high-frequency acoustic microscopy coupled with a generative AI model to map and analyze the 3D acoustic signature of the interior of a finished chip (without opening it). The model compares this fingerprint to the original “Golden Layout” to detect the slightest nanometric alteration."]
```
