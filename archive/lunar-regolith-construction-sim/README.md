...
```mermaid
graph TD
    A["Lunar Regolith Properties (Electrostatic, Abrasive, Vacuum)"] --> B{"Granular Dynamics Engine (DEM + AI)"}
    B -->|Low-Gravity Simulation| C["Virtual Sim-to-Real Environment"]
    C --> D["Reinforcement Learning Training for Construction Robots"]
    D --> E["Autonomous In-Situ Resource Utilization (ISRU) on Moon/Mars"]
```
...
```mermaid
sequenceDiagram
    participant Eng as Robotics Engineer
    participant Sim as Neural Granular Simulator
    participant RL as Reinforcement Learning Agent
    Eng->>Sim: Define robotic morphology & task (e.g. 3D print habitat wall)
    Sim->>Sim: Calculate regolith interaction (Vacuum, 1/6G, Electrostatics)
    Sim->>RL: Expose granular states to AI agent
    RL->>Sim: Execute kinematic action (dig, pour, print)
    Sim-->>RL: Feedback (Jamming, spillage, structural integrity)
    RL->>Eng: Output optimized control policy for physical robot deployment
```
