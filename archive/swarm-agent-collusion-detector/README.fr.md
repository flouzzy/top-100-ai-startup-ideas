...
```mermaid
graph TD
    A["Agents IA de Trading Autonomes"] -->|Stratégies Coopératives Cachées| B["Collusion Algorithmique / Cartel"]
    B -->|Manipulation Subtile du Marché| C["Indétectable par la Compliance Standard"]
    D["Moteur Théorie des Jeux Inverse"] -->|Analyse Graphes Dynamiques| E["Détection de Motifs Comportementaux"]
    E -->|Alerte Régulateurs & Plateformes| F["Prévention de Défaillance Systémique"]
```
...
```mermaid
sequenceDiagram
    participant Agents as "Essaim IA Trading"
    participant Engine as "Détecteur Collusion"
    participant GT as "Modèle Théorie des Jeux Inverse"
    participant Compliance as "Compliance/Régulateur"

    Agents->>Engine: Flux données transactions haute fréquence
    Engine->>GT: Analyse espaces de décision & politiques RL
    GT->>GT: Détection motifs d'équilibre non-concurrentiels
    GT-->>Engine: Signalement de stratégies coopératives cachées
    Engine-->>Compliance: Alerte avec preuve mathématique d'intentionnalité
```
